            ...
            <ScrollView contentContainerStyle={styles.scrollContent}>
              {Platform.OS === 'android' && (
                <Pressable onPress={() => setShowDate(true)}>
                  <View style={styles.inputContainer}>
                    ...
                    <MaterialIcons name={'touch-app'} size={30} color="#000" style={{ marginLeft: -40 }} />
                    {showDate && <DateTimePicker display="spinner" value={date} mode="date" onChange={onDateChange} />}
                  </View>
                </Pressable>
              )}
              {Platform.OS === 'ios' && (
                <View style={styles.datePickerContainer}>
                  ...
                  <DateTimePicker display="default" value={date} mode="date" onChange={onDateChange} />
                </View>
              )}
              {answers.map((answer, index) => (
                <View key={index}>
                  ...
                  {answer.symptom === '其他' ? (
                    <TextInput style={styles.input} placeholder="請輸入症狀" value={answer.customSymptom || ''} onChangeText={changeCustomSymptom} />
                  ) : (
                    <View style={styles.optionsContainer}>
                      <TouchableOpacity style={[styles.optionButton, styles.optionLeft, answer.hasSymptom && styles.selectedOption]} onPress={() => toggleSymptom(index, true)}>
                        <Text style={styles.optionText}>有症狀</Text>
                      </TouchableOpacity>
                      <TouchableOpacity style={[styles.optionButton, styles.optionRight, !answer.hasSymptom && styles.selectedOption]} onPress={() => toggleSymptom(index, false)}>
                  ...
                  {(answer.hasSymptom || answer.symptom === '其他') && (
                    <View style={styles.severityContainer}>
                      {[0, 1, 2, 3].map((severity) => (
                        <TouchableOpacity key={severity} style={[styles.severityButton, answer.severity === severity && styles.selectedSeverity]} onPress={() => changeSeverity(index, severity)}>
                          <Text style={styles.severityText}>{severity === 0 ? '無' : severity === 1 ? '小' : severity === 2 ? '中' : '大'}</Text>
                        </TouchableOpacity>
                      ))}
                    </View>
                  )}
...
const BottomTabs: React.FC<BottomTabsProps> = ({ role, customedStyle }) => {
  ...
  return (
    <SafeAreaView edges={['bottom']} style={[bottomsList.bottomSafeview, { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: 'white' }]}>
